...
function renderRoleSubtitle(candidate: Candidate): string {
  const role = candidate.currentRole?.trim()
  const company = candidate.currentCompany?.trim()
  if (!role && !company) return ''
  if (role && company) return `${esc(role)} at ${esc(company)}`
  return esc(role || company)
}

function renderCard(candidate: Candidate, slug: string): string {
  ...
  <div class="cand-header">
    ...
    <div class="cand-header-info">
      ...
      <div class="cand-sub">${renderRoleSubtitle(candidate)}</div>
    </div>
  </div>
...
function renderHardFactorsNote(note: string | undefined): string {
  if (!note?.trim()) return ''
  return `<div class="lb">Scoring based on hard facts (CV)</div>
<div class="gd" style="margin:20px 0">
  <p style="margin-bottom:0">${esc(note)}</p>
</div>`
}

function renderGdprDisclaimer(): string {
  // Default disclaimer — always shown. Future: optional override field on the
  // section if firms want different wording.
  return `<div style="border-left:3px solid var(--sand);padding:12px 18px;margin:20px 0 24px 0;font-size:13px;font-style:normal;color:var(--txt3);line-height:1.6;font-family:var(--sans)">In compliance with GDPR regulations, we share only publicly available information about candidates until we have their explicit consent for introduction. Due to the confidential nature of this search, no candidates have been approached at this stage.</div>`
}
...
export function renderCandidates(
  ...
  if (data.candidates.length === 0) {
    return `<div class="ot-empty">No candidates added yet.</div>`
  }
  ...
  const cards = sorted
    .map((c) => renderCard(c, slugMap.get(c.id) ?? c.id))
    .join('\n')

  return `${renderHardFactorsNote(data.hardFactorsGateNote)}
${renderGdprDisclaimer()}
